...
export type MiniGameResult = { status: "SUCCESS" | "FAILED" | "CANCELLED"; score: number; route: "EXPERT" | "INVESTIGATION" | "GENERAL" };
...
export function evaluatePattern(answer: string, hasBonusClue: boolean) {
  return answer.trim().toLowerCase() === (hasBonusClue ? "triangle" : "triangle");
}
export function evaluateEvidenceOrder(order: string[]) {
  return order.join(",") === "courtyard,library,final";
}
export function evaluateTiming(progress: number, targetStart = 42, targetEnd = 58) {
  return progress >= targetStart && progress <= targetEnd;
}
